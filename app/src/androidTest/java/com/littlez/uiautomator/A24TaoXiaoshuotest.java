 ...
 * Date 2019/12/3
 */
public class A24TaoXiaoshuotest extends TestCase {

    /*app 名字*/
    private String appName = "淘小说";
    ...
    public void test() throws UiObjectNotFoundException {
        ...
        try {
            while (true) {
                //主页
                UiObject uiMain = new UiObject(new UiSelector().resourceId("com.martian.ttbook:id/bottom_navigation_bar"));
                UiObject uiReadPage = new UiObject(new UiSelector().resourceId("com.martian.ttbook:id/drawer_layout"));
                UiObject uiWelare = new UiObject(new UiSelector().resourceId("com.dragon.read:id/io"));

                if (uiMain.exists()) {//是主页
                    UiObject uiBooks = new UiObject(new UiSelector().resourceId("com.martian.ttbook:id/bookrack_irc"));
                    if (uiWelare.isChecked()) {//选中的福利
                        welare(uiDevice);   //福利页面
                    } else if (uiBooks.exists()) {//选中的书架
                        UiObject uiBookTitle =
                                new UiObject(new UiSelector().resourceId("com.martian.ttbook:id/bookrack_grid_bookname"));
